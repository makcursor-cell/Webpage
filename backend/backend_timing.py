...
# Helper: make HTML links for DataFrame cells
...
def get_design_name(common_setup_path):
    """Extract DESIGN_NAME from common_setup.tcl file."""
    if not os.path.exists(common_setup_path):
        return None
    try:
        with open(common_setup_path, encoding='utf-8', errors='ignore') as f:
            for line in f:
        ...
        import logging
        logging.warning(f"Error reading {common_setup_path}: {e}")
    return None

def write_comparison_html(df, outdir):
    """Write comparison dashboard HTML."""
    ...
    try:
        with open(os.path.join(outdir, "comparison_dashboard.html"), "w", encoding='utf-8') as f:
        f.write("""
        <html>
        ...
        import logging
        logging.error(f"Error writing comparison dashboard to {outdir}: {e}", exc_info=True)
        raise
...
def parse_timing(report_path):
    """Parse timing report to extract setup and hold violations."""
    ...
    try:
        with open(report_path, encoding='utf-8', errors='ignore') as f:
            for line in f:
                l = line.strip()
                if l == "Setup violations": 
                    mode, header_seen = "setup", False
                    continue
                if l == "Hold violations": 
                    mode, header_seen = "hold", False
                    continue
                if "No setup violations found" in l or "No hold violations found" in l: 
                    mode = None
                    continue
                if not mode: 
                    continue
        ...
        import logging
        logging.warning(f"Error parsing timing report {report_path}: {e}")
    except Exception as e:
        import logging
        logging.error(f"Unexpected error parsing timing report {report_path}: {e}", exc_info=True)
    return ([setup["WNS"], setup["TNS"], setup["NUM"]], [hold["WNS"], hold["TNS"], hold["NUM"]])

def extract_report_date(report_path):
    """Extract date from report file header."""
    ...
    try:
        ...
        import logging
        logging.warning(f"Error reading report date from {report_path}: {e}")
    except Exception as e:
        import logging
        logging.debug(f"Error extracting date from {report_path}: {e}")
    return ""

def extract_simple_count(path, keyword):
    """Count lines containing a keyword in a file."""
...
def extract_freq(report_path):
    """Extract frequency value from report."""
    try:
        with open(report_path, encoding='utf-8', errors='ignore') as f:
            for line in f:
        ...
        import logging
        logging.warning(f"Error extracting frequency from {report_path}: {e}")
    except Exception as e:
        import logging
        logging.debug(f"Error extracting frequency: {e}")
    return ""
...
def gen_histogram(report_path, die, design_id):
    """Generate histogram data from timing violation report."""
    arr = []
    try:
        with open(report_path, encoding='utf-8', errors='ignore') as f:
            for line in f:
                if "(VIOLATED)" in line and "slack" in line.lower():
                    ...
                    if nums: 
                        try:
                            arr.append(abs(float(nums[-1])))
                        except (ValueError, IndexError):
                            continue
    except (IOError, OSError) as e:
        import logging
        logging.warning(f"Error reading histogram data from {report_path}: {e}")
        return None
    except Exception as e:
        import logging
        logging.error(f"Unexpected error generating histogram from {report_path}: {e}", exc_info=True)
        return None
    ...
    try:
        ...
        import logging
        logging.error(f"Error creating histogram DataFrame: {e}", exc_info=True)
        return None
...
def write_html(obj, name, outdir, report_date=""):
    ...
    try:
        ...
        import logging
        logging.error(f"Error writing HTML file {name}.html to {outdir}: {e}", exc_info=True)
        raise
...
# Main entry: generate HTML dashboards for designs in a chip
